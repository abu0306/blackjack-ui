...
function App() {
  ...
  useEffect(() => {
    const nodeConnection = new AleoNetworkClient("http://192.168.200.25:3030");
	  const nodeRoot = 'zkgaming_blackjack.aleo';
    let stage = new createjs.Stage(canvasRef.current);
	  ...
	  var game = {
		  ...
		  buttons: [
			// new Button('Hit', '#fff', 100, 100, () =>game.init_test()),
		  ...
		  go: function () {
			  if (player.dealt && !this.inProgress) {
				  ...
				  //todo 
				  mask.show(`./target/release/snarkos developer execute --private-key ${player.account_key} --broadcast "http://127.0.0.1:3030/testnet3/transaction/broadcast" --query "http://127.0.0.1:3030"  ${nodeRoot} request_start`);
				  mask.mask_init();
		...
		distributeCard: function (to, hidden = false, value) {
			...
			console.log('=card==number3',number)
	...
	var bank = {
		...
		play: function(cardList,winer,nextIndex){
			...
			var bankScore = game.deckValue(bank.deck);
			var playerScore = game.deckValue(player.deck);

			console.log("=======bank play===========+++",bankScore,playerScore,winer,record_state.account.toString(),Number(item_card_value))
			if (winer === record_state.account.toString()) {
				/// player winner
				if (bankScore + Number(item_card_value.toString()) <= playerScore) {
				...
				} else if (bankScore + Number(item_card_value.toString()) >= playerScore && !this.blackJack) {
					this.blackJack = true
					game.distributeCard('bank',undefined,item_card_value.toString());
					bank.play(
						cardList,winer,nextIndex
					)
				}
				else {
					game.check();
	...
	var player = {
    	...
    	hit: function () {
			mask.show(`./target/release/snarkos developer execute --private-key ${player.account_key} --broadcast "http://127.0.0.1:3030/testnet3/transaction/broadcast" --query "http://127.0.0.1:3030" ${nodeRoot} request_hit`);
			mask.mask_init();
		...
		stand_card: function (cardList, winer, nextIndex) { 
			console.log("===stand_card====",this.betted)
			if(!this.betted)
		...
		stand: function () {
			mask.show(`./target/release/snarkos developer execute --private-key ${player.account_key} --broadcast "http://127.0.0.1:3030/testnet3/transaction/broadcast" --query "http://127.0.0.1:3030" ${nodeRoot} request_stand`);
			mask.mask_init();
        ...
        if (transaction.type === "execute") {
          transaction.execution.transitions.forEach((transition) => {
            ...
            if (transition.function === "exchange") {
              console.log('=====45645665-currentHeight--',transition)
              ...
              if (recordCiphertext.isOwner(this.account.viewKey())) {
				...
				console.log('---3,',exchange.owner,this.account.toString())
                if (exchange.owner === this.account.toString() ) {
                  //设置游戏ID
					this.gameId = exchange.id;
					mask.show(`./target/release/snarkos developer execute --private-key ${player.account_key} --broadcast "http://127.0.0.1:3030/testnet3/transaction/broadcast" --query "http://127.0.0.1:3030" ${nodeRoot} set_part2_nonce   "${recordPlaintext.toString()}" ${parseInt(Math.random()*10000+ 5000)}u64`)
                    mask.mask_init();
				  ...
				  transition.outputs.forEach((value, index) => {
					...
					if (index === 2) {
						player_first = {
							mainer: 'player',
							hidden:false,
							value: marker_card.raw_point.replace(/\u128/g,''),
						}
					} else { 
						 card_value.push({
            ...
            if (transition.function === "process_hit_request") {
				...
				console.log('-out--process_hit_request',out,this.gameId,record_state.account.toString())
              if (out.player === record_state.account.toString() && out.game_id === this.gameId) {
				  ...
				  player.hit_card({
							mainer: 'player',
							hidden:false,
							value: my_card.raw_point.replace(/\u128/g,''),
						},isEnd)
