...
export class TextInput extends createjs.Container {
  ...
  _setupDomNode() {
    ...
    this._hiddenInput.style.overflow = 'hidden';
    this._hiddenInput.style.whiteSpace = 'nowrap';
    this._hiddenInput.style.textOverflow = 'ellipsis';
  ...
  _setupListeners() {
    ...
    this._hiddenInput.addEventListener('input', (e) => {
      if (this._focused) {
        ...
        const pro_text = ctx.measureText(this._preCursorText);
        if (pro_text.width > 100) { 
          this._visibleText = this._preCursorText;
          this._preCursorText = `${this._preCursorText.slice(0, 6)}...${this._preCursorText.slice(-6)}`;
        }
        this.update();
